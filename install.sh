...
install_docker() {
    ...
    # Modified for WSL + Docker Desktop integration support
    if ! docker info &>/dev/null; then
        color_echo "[Error] Docker is not running. Please ensure Docker Desktop is running on Windows and WSL integration is turned on for your Parrot OS." red >&2
        exit 1
    else
        color_echo "[OK] Docker service is running (WSL Integration)"
    fi
}
